...
public class PubSubBigQuery extends InputOutput {
  ...
  public void writePredictions(PCollection<Record> instances, MyOptions options) {
    ...
    PCollection<TransactionPred> preds = addPredictionInBatches(instances);
  ...
  private PCollection<TableRow> toTableRows(PCollection<TransactionPred> preds) {
    return preds.apply("pred->row", ParDo.of(new DoFn<TransactionPred, TableRow>() {
      @ProcessElement
      public void processElement(ProcessContext c) throws Exception {
        ...
        if (pred.predictedFraudProbability >= 0) {
          row.set("predicted_transaction_fraud", Math.round(pred.predictedFraudProbability*100)/100.0);
        }
        c.output(row);
      }})) //
        ;
  ...
  public PubSubBigQuery() {
    ...
    types[INPUTCOLS.fraud_probability.ordinal()] = "FLOAT";
    types[INPUTCOLS.amount.ordinal()] = "FLOAT";
    types[INPUTCOLS.balance.ordinal()] = "FLOAT";
  }
  ...
  private List<TableFieldSchema> getTableFields() {
    ...
    fields.add(new TableFieldSchema().setName("predicted_transaction_fraud").setType("FLOAT"));
    return fields;
  }    
}
